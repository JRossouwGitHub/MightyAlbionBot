...
client.on('messageCreate', (message) => {
    ...
    const nickname = message.guild.members.cache.map(member => { if(member.user.username == message.author.username) return member.nickname.length < 1 ? message.author.username : member.nickname}).toString().replace(/,/g, "")
    switch(command){
        case "ping":
...
client.on(Events.MessageReactionAdd, async (reaction, user) => {
    ...
    const nickname = reaction.message.guild.members.cache.map(member => { if(member.user.username == user.username) return member.nickname.length < 1 ? user.username : member.nickname}).toString().replace(/,/g, "")
